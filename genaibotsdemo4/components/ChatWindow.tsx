...
//  emoji?: string;
  ...
  return (
    <div className={`flex flex-col items-left p-4 md:p-8 rounded grow overflow-hidden ${(messages.length > 0 ? "border" : "border")}`}>
      <h2 className={`${messages.length > 0 ? "" : ""} text-xl`}>{titleText}</h2>
      {messages.length === 0 ? emptyStateComponent : ""}
      <div
        className="flex flex-col-reverse w-full mt-2 overflow-auto transition-[flex-grow] ease-in-out"
      ...
      <form onSubmit={sendMessage} className="flex w-full flex-col">
        ...
        <div className="flex w-full mt-2">
          <input
            className="grow mr-8 p-3 rounded"
            value={input}
            placeholder={placeholder ?? "Can you convert 1000 dollards in EUR"}
            onChange={handleInputChange}
          />
          <button type="submit"  className={"shrink-0 px-8 py-1 bg-blue-600 rounded w-18 test"}>
            ...
            <span className={(chatEndpointIsLoading || intermediateStepsLoading) ? "hidden" : "text-xs"}><Send className="h-6 w-6 text-white" /></span>
          </button>
        </div>
      </form>
      <ToastContainer/>
